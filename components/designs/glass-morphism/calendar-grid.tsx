...
const typeTone: Record<string, { gradient: string; bg: string }> = {
  cron: { gradient: "from-emerald-400 to-teal-500", bg: "bg-emerald-400/20" },
  reminder: { gradient: "from-amber-400 to-orange-500", bg: "bg-amber-400/20" },
  recurring: { gradient: "from-sky-400 to-blue-500", bg: "bg-sky-400/20" },
};
    ...
    <section className="space-y-4">
      ...
      <div className="rounded-3xl border border-white/20 bg-white/10 p-5 shadow-xl backdrop-blur-xl">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-xs font-medium text-white/50">Planning Window</p>
            <h2 className="mt-1 text-2xl font-semibold text-white">{weekLabel}</h2>
          </div>
          <div className="flex flex-wrap gap-2">
            {[
              { label: "Prev", onClick: () => { setWeekStart((c) => addDays(c, -7)); setMobileOffset(0); }, icon: ChevronLeft },
              { label: "Today", onClick: () => { setWeekStart(startOfWeek(new Date())); setMobileOffset(getMobileOffsetForDate(new Date())); }, icon: null },
              { label: "Next", onClick: () => { setWeekStart((c) => addDays(c, 7)); setMobileOffset(0); }, icon: ChevronRight },
            ].map((btn) => (
              <button
                key={btn.label}
                onClick={btn.onClick}
                className="flex items-center gap-1 rounded-xl bg-white/10 px-4 py-2 text-sm font-medium text-white/80 transition-all hover:bg-white/20"
              >
                {btn.icon === ChevronLeft && <ChevronLeft className="size-4" />}
                {btn.label}
                {btn.icon === ChevronRight && <ChevronRight className="size-4" />}
              </button>
            ))}
            <button
              onClick={handleSeed}
              disabled={seedState === "pending"}
              className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-500 px-4 py-2 text-sm font-medium text-white shadow-lg shadow-violet-500/30 transition-all hover:shadow-violet-500/50 disabled:opacity-50"
            >
      ...
      {/* Mobile 3-day */}
      <div className="rounded-3xl border border-white/20 bg-white/10 p-3 shadow-xl backdrop-blur-xl md:hidden">
        <div className="mb-3 flex items-center justify-between">
          <span className="text-xs font-medium text-white/60">3-Day View</span>
          <div className="flex gap-1">
            <button
              onClick={() => setMobileOffset((v) => Math.max(0, v - 1))}
              disabled={mobileOffset === 0}
              className="rounded-lg bg-white/10 p-1.5 text-white/60 disabled:opacity-30"
            >
...
function PlannerBoard({
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-white/20 bg-white/5 shadow-xl backdrop-blur-xl">
      <div className="overflow-x-auto">
        <div className="min-w-[700px]">
          {/* Header */}
          <div className="grid border-b border-white/10" style={{ gridTemplateColumns: `64px repeat(${days.length}, minmax(130px, 1fr))` }}>
            <div className="border-r border-white/10 p-3 text-right text-[10px] font-medium text-white/40">Time</div>
            {days.map((day) => {
              const isToday = dayKey(day) === dayKey(now);
              return (
                <div key={dayKey(day)} className={cn("border-r border-white/10 p-3", isToday && "bg-white/10")}>
                  <p className="text-xs font-semibold text-white/90">{day.toLocaleDateString("en-US", { weekday: "short" })}</p>
                  <p className="text-[11px] text-white/50">{day.toLocaleDateString("en-US", { month: "short", day: "numeric" })}</p>
                </div>
              );
            })}
          </div>
          ...
          {/* Grid */}
          <div className="grid" style={{ gridTemplateColumns: `64px repeat(${days.length}, minmax(130px, 1fr))` }}>
            <div className="relative border-r border-white/10" style={{ height: `${DAY_HEIGHT}px` }}>
              {HOURS.map((hour) => (
                <div key={hour} className="absolute left-0 right-0 border-t border-white/5 text-right text-[10px] text-white/30" style={{ top: `${hour * SLOT_HEIGHT}px` }}>
            ...
            {days.map((day) => {
              ...
              return (
                <div key={key} className={cn("relative border-r border-white/10", isToday && "bg-white/5")} style={{ height: `${DAY_HEIGHT}px` }}>
                  {HOURS.map((hour) => (
                    <div key={hour} className="absolute left-0 right-0 border-t border-white/5" style={{ top: `${hour * SLOT_HEIGHT}px` }} />
                  ))}
                  ...
                  {dayTasks.map((task) => {
                    ...
                    const tone = typeTone[task.type] ?? { gradient: "from-gray-400 to-gray-500", bg: "bg-gray-400/20" };
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "absolute left-1 right-1 overflow-hidden rounded-xl border border-white/20 p-2 text-left backdrop-blur-sm transition-all hover:scale-[1.02] hover:shadow-lg",
                          tone.bg,
                      ...
                      >
                        ...
                        <p className="text-[10px] text-white/60">{startDate.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                  ...
                  {nowTop !== null && (
                    <div className="pointer-events-none absolute left-0 right-0 z-10" style={{ top: `${nowTop}px` }}>
                      <div className="absolute -left-1 top-0 size-2.5 rounded-full bg-gradient-to-r from-rose-400 to-pink-500 shadow-lg shadow-rose-500/50" />
  ...
  return (
    <div className={cn("fixed inset-0 z-50 transition", isOpen ? "pointer-events-auto" : "pointer-events-none")}>
      <button
        type="button"
        onClick={onClose}
        className={cn("absolute inset-0 bg-black/50 backdrop-blur-sm transition-opacity", isOpen ? "opacity-100" : "opacity-0")}
      />
      <aside
        className={cn(
          "absolute inset-y-0 right-0 w-full max-w-md border-l border-white/20 bg-white/10 p-6 shadow-2xl backdrop-blur-2xl transition-transform",
        ...
        {task && (
          <div className="flex h-full flex-col">
            <div className="mb-6 flex items-start justify-between">
              <div>
                <p className="text-xs font-medium text-white/50">Task Details</p>
                <h3 className="mt-1 text-xl font-semibold text-white">{task.name}</h3>
              </div>
              <button onClick={onClose} className="rounded-xl bg-white/10 p-2 text-white/60 hover:bg-white/20 hover:text-white">
                <X className="size-5" />
              </button>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <p className="text-xs font-medium text-white/50">Config</p>
                <pre className="mt-2 max-h-48 overflow-auto rounded-xl bg-black/30 p-3 text-xs text-white/70">
...
function formatDateTime(ts?: number) { if (!ts) return "Not recorded"; return new Date(ts).toLocaleString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" }); }
function formatConfig(config: unknown) { if (!config) return "No config"; try { return JSON.stringify(config, null, 2); } catch { return String(config); } }
function formatHour(hour: number) { if (hour === 0) return "12a"; if (hour < 12) return `${hour}a`; if (hour === 12) return "12p"; return `${hour - 12}p`; }
function minuteToOffset(minutes: number) { return (minutes / 60) * SLOT_HEIGHT; }
function minutesToHeight(minutes: number) { return Math.max((minutes / 60) * SLOT_HEIGHT, 28); }
