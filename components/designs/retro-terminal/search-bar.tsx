...
function highlightMatch(text: string, query: string) {
  ...
  return text.split(pattern).map((seg, i) =>
    seg.toLowerCase() === query.toLowerCase() ? (
      <mark key={i} className="bg-[#33ff33] text-black">{seg}</mark>
    ) : (
...
function getActivityTypeIcon(type: string): LucideIcon {
  const icons: Record<string, LucideIcon> = { email: Mail, code: Code2, cron: Clock3, search: Search, message: MessageSquare, file: File, browser: Globe };
...
export function SearchBar({ widget = false, className }: SearchBarProps) {
  ...
  const onInputKeyDown = useCallback((event: ReactKeyboardEvent<HTMLInputElement>) => {
    if (event.key === "ArrowDown") { event.preventDefault(); if (flatResults.length) setActiveIndex((p) => Math.min((p < 0 ? 0 : p + 1), flatResults.length - 1)); }
    if (event.key === "ArrowUp") { event.preventDefault(); if (flatResults.length) setActiveIndex((p) => Math.max(0, p - 1)); }
    if (event.key === "Enter" && clampedActiveIndex >= 0) {
      event.preventDefault();
      const key = makeResultKey(flatResults[clampedActiveIndex].group, flatResults[clampedActiveIndex].item);
  ...
  const renderResults = () => {
    ...
    return (
      <div className="space-y-3">
        {displayIsLoading && (
          <div className="flex items-center gap-2 rounded border border-[#33ff33]/30 p-3 text-xs">
            <Loader2 className="size-3 animate-spin" />
            <span>grep -r &quot;{debouncedQuery}&quot; ...</span>
          </div>
        )}

        {displayError && (
          <div className="rounded border border-[#ff3333]/50 bg-[#ff3333]/10 p-3 text-xs text-[#ff3333]">{displayError}</div>
        )}

        {isIdle && (
          <div className="space-y-3">
            {history.length > 0 && (
              <section className="rounded border border-[#33ff33]/30 bg-black p-3">
                <div className="mb-2 flex items-center gap-2 text-[10px] text-[#33ff33]/50">
                  <History className="size-3" />
                  history
                </div>
                <div className="flex flex-wrap gap-1">
                  {history.map((entry) => (
                    <button key={entry} onClick={() => applySuggestion(entry)} className="rounded border border-[#33ff33]/30 px-2 py-1 text-[10px] hover:border-[#33ff33] hover:bg-[#33ff33]/10">
                      {entry}
                    </button>
                  ))}
                </div>
            ...
            <section className="rounded border border-[#33ff33]/30 bg-black p-3">
              <p className="text-[10px] text-[#33ff33]/50"># tips</p>
              <ul className="mt-1 space-y-0.5 text-[10px] text-[#33ff33]/70">
                <li>- exact match: &quot;daily report&quot;</li>
                <li>- cron patterns: */15</li>
                <li>- paths: /home/.claude</li>
              </ul>
            </section>
            ...
            <section className="rounded border border-[#33ff33]/30 bg-black p-3">
              ...
              <div className="space-y-1">
                {(recentActivities ?? []).slice(0, widget ? 3 : 5).map((a: Doc<"activities">) => (
                  <article key={a._id} className="rounded border border-[#33ff33]/20 bg-[#33ff33]/5 p-2 text-[10px]">
                    <p>{a.title}</p>
                    <p className="text-[#33ff33]/50">{a.type} | {formatTimestamp(a.timestamp)}</p>
                  </article>
                ))}
              </div>
            </section>
        ...
        {!displayIsLoading && shouldSearch && displayResults && total === 0 && (
          <div className="rounded border border-dashed border-[#33ff33]/30 p-6 text-center text-xs text-[#33ff33]/50">
            <FileSearch className="mx-auto size-8" />
            <p className="mt-2">grep: no matches found</p>
          </div>
        )}

        {!displayIsLoading && shouldSearch && displayResults && total > 0 && (
          <div className="space-y-3">
            <ResultSection group="activities" title="activities" icon={Radar} count={displayResults.activities.length} collapsed={collapsed.activities} onToggle={() => setCollapsed((p) => ({ ...p, activities: !p.activities }))}>
              {displayResults.activities.map((item) => { const key = makeResultKey("activities", item); const idx = resultIndexMap.get(key) ?? -1; return <ResultCard key={key} index={idx} query={debouncedQuery} item={item} active={idx === clampedActiveIndex} expanded={expandedKey === key} onSelect={() => { setActiveIndex(idx); setExpandedKey((p) => (p === key ? null : key)); }} />; })}
            </ResultSection>
            <ResultSection group="tasks" title="scheduled" icon={Clock3} count={displayResults.tasks.length} collapsed={collapsed.tasks} onToggle={() => setCollapsed((p) => ({ ...p, tasks: !p.tasks }))}>
              {displayResults.tasks.map((item) => { const key = makeResultKey("tasks", item); const idx = resultIndexMap.get(key) ?? -1; return <ResultCard key={key} index={idx} query={debouncedQuery} item={item} active={idx === clampedActiveIndex} expanded={expandedKey === key} onSelect={() => { setActiveIndex(idx); setExpandedKey((p) => (p === key ? null : key)); }} />; })}
            </ResultSection>
            <ResultSection group="documents" title="documents" icon={FileText} count={displayResults.documents.length} collapsed={collapsed.documents} onToggle={() => setCollapsed((p) => ({ ...p, documents: !p.documents }))}>
              {displayResults.documents.map((item) => { const key = makeResultKey("documents", item); const idx = resultIndexMap.get(key) ?? -1; return <ResultCard key={key} index={idx} query={debouncedQuery} item={item} active={idx === clampedActiveIndex} expanded={expandedKey === key} onSelect={() => { setActiveIndex(idx); setExpandedKey((p) => (p === key ? null : key)); }} />; })}
            </ResultSection>
          </div>
        )}
      </div>
  ...
  const renderPrompt = (inputRef: RefObject<HTMLInputElement | null>, options?: { mobileMode?: boolean; onClose?: () => void }) => {
    ...
    return (
      <div className="space-y-3 font-mono text-[#33ff33]">
        <section className={cn("rounded border border-[#33ff33]/30 bg-black", widget ? "p-3" : "p-4")}>
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-[10px] text-[#33ff33]/50">$ grep -r</p>
              <p className="mt-0.5 text-xs text-[#33ff33]/70">unified search</p>
            </div>
            {!mobileMode && <span className="rounded border border-[#33ff33]/30 px-2 py-1 text-[9px]">^K</span>}
            {mobileMode && options?.onClose && (
              <button onClick={options.onClose} className="rounded border border-[#33ff33]/30 px-2 py-1 text-xs">close</button>
            )}
          </div>

          <div className="mt-3 flex items-center rounded border border-[#33ff33]/50 bg-black">
            <span className="px-2 text-[#33ff33]/50">$</span>
            <input
              ref={inputRef}
              value={query}
              ...
              className="h-10 flex-1 bg-transparent text-sm placeholder:text-[#33ff33]/30 focus:outline-none"
            />
            <span className="animate-pulse px-2">_</span>
          </div>

          <div className="mt-2 flex gap-3 text-[9px] text-[#33ff33]/40">
            <span>↑↓:nav</span>
            <span>enter:expand</span>
            <span>{displayResults?.total ?? 0} hits</span>
          </div>
        </section>
    ...
    <section className={cn("space-y-3", className)}>
      ...
      <div className="md:hidden">
        <button
          ...
          className="flex w-full items-center justify-between rounded border border-[#33ff33]/30 bg-black px-3 py-2 font-mono text-[#33ff33]"
        >
          <span className="text-xs">$ grep {query.trim() || "..."}</span>
          <span className="text-[9px] text-[#33ff33]/50">tap</span>
        </button>
      </div>

      {isMobileOverlayOpen && (
        <div className="fixed inset-0 z-[90] overflow-y-auto bg-black p-3 pb-20 md:hidden">
...
function ResultSection({ group, title, icon: Icon, count, collapsed, onToggle, children }: { group: GroupKey; title: string; icon: LucideIcon; count: number; collapsed: boolean; onToggle: () => void; children: ReactNode }) {
  return (
    <section className="rounded border border-[#33ff33]/30 bg-black p-3" data-group={group}>
      <button type="button" onClick={onToggle} className="flex w-full items-center justify-between gap-3 border-b border-[#33ff33]/20 pb-2">
        <span className="flex items-center gap-2 text-xs">
          <Icon className="size-3" />
          {title}
        </span>
        <span className="flex items-center gap-2 text-[10px]">
          <span className="rounded bg-[#33ff33]/20 px-1.5">{count}</span>
          {collapsed ? <ChevronRight className="size-3" /> : <ChevronDown className="size-3" />}
        </span>
      </button>
      {!collapsed && <div className="mt-2 space-y-1">{children}</div>}
    </section>
  );
}
...
function ResultCard({ item, query, index, active, expanded, onSelect }: { item: SearchResult; query: string; index: number; active: boolean; expanded: boolean; onSelect: () => void }) {
  ...
  return (
    <button
      ...
      onClick={onSelect}
      className={cn(
        "w-full rounded border p-2 text-left text-[10px] transition-all",
        active ? "border-[#33ff33] bg-[#33ff33]/10" : "border-[#33ff33]/20 hover:border-[#33ff33]/50"
      )}
    >
      <div className="flex items-start gap-2">
        <span className="mt-0.5 text-[#33ff33]/50">{iconElement}</span>
        <div className="min-w-0 flex-1">
          <p className="truncate">{highlightMatch(item.title, query)}</p>
          <p className="mt-0.5 line-clamp-1 text-[9px] text-[#33ff33]/50">{highlightMatch(item.snippet, query)}</p>
        </div>
        <span className="shrink-0 text-[9px] text-[#33ff33]/40">[{item.subtype}]</span>
      </div>

      <div className="mt-1 flex items-center gap-2 text-[8px] text-[#33ff33]/40">
        <span>{formatTimestamp(item.timestamp)}</span>
        <span className="h-1 w-8 bg-[#33ff33]/20">
          <span className="block h-full bg-[#33ff33]" style={{ width: `${Math.round(item.relevance * 100)}%` }} />
        </span>
      </div>

      {expanded && (
        <div className="mt-2 border-t border-[#33ff33]/20 pt-2 text-[9px] text-[#33ff33]/60">
          {item.source === "activities" && <p>status: {item.status}</p>}
          {item.source === "scheduledTasks" && <><p>schedule: {item.schedule}</p><p>enabled: {item.enabled ? "true" : "false"}</p></>}
